...
unsigned int nTargetSpacing = 1 * 110; // 110 seconds
...
const string strMessageMagic = "GoldReserve Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 500)
      {
        int64_t nSubsidy = 300 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight+1 <= 5000)
      {
        int64_t nSubsidy = 600 * COIN;
        return nSubsidy + nFees;
      }
...
static const int64_t nTargetTimespan = 10 * 110;  // 10 blocks
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GoldReserve", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Gold Reserve";
        CTransaction txNew;
        txNew.nTime = 1401907836;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401907836;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3451915;
		if(fTestNet)
        {
            block.nNonce   = 3451915;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x45bf9761d8c701bd5ab6d1e3f8bfd9c47c844e928e184517d93903f4762ebc36"));
        block.print();
